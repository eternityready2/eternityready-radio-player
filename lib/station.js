...
export const createStation = async (formData) => {
  try {
    ...
    if (stationId) {
      await query(
        "UPDATE stations SET isDefault = ?, isActive = ?, name = ?, refUrl = ?, url = ?, donateLink = ?, gtm = ?, analytics = ?, advertisements = ?, logo = ?, thumbnail = ?, backgroundImage = ? WHERE id = ?",
        [
          station.isDefault,
          station.isActive,
          station.name,
      ...
      const results = await query(
        "INSERT INTO stations (isDefault, isActive, name, refUrl, url, donateLink, gtm, analytics, advertisements, logo, thumbnail, backgroundImage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          station.isDefault,
          station.isActive,
          station.name,
